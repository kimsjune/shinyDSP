...
.volcanoFunction <- function(volcano, delabSize, maxOverlap, title,
    ...
    ggplot2::ggplot(data=volcano,
        ...
        ggplot2::geom_vline(xintercept=c(-(logFCcutoff),logFCcutoff), col="black",
                   linetype=3)+
        ggplot2::geom_hline(yintercept=-log10(PvalCutoff), col="black",
                   linetype=3)+
        ...
        ggplot2::scale_color_manual(values=c(DnCol, notDEcol, UpCol),
                           breaks=c("DN","NA","UP"))+
        ggplot2::xlab(expression(log[2]~fold~change))+
        ggplot2::ylab(expression(-log[10]~italic(P)~value))+
        ggplot2::theme(aspect.ratio=1, rect=ggplot2::element_rect(fill="transparent"))+
